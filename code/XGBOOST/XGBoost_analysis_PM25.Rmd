...
vars <- fread("/Volumes/GoogleDrive/My Drive/AERONET-MISR/Paper#2-RSoE/aeronet/aeronet/data/inv.vars2.csv")
...
# regional subsets
wdb.f.w <-wdb.f[wdb.f$Site_Name %in% c("Fresno_2", "DRAGON_Madera_city", "DRAGON_Tranquility","DRAGON_Bakersfield"),]
sd(wdb.f.w$PM25)
wdb.f.c <-wdb.f[wdb.f$Site_Name %in% c("Denver_LaCasa", "DRAGON_Chatfield_Pk"),]
sd(wdb.f.c$PM25)
wdb.f.e <-wdb.f[wdb.f$Site_Name %in% c("CCNY","USDA_Howard","DRAGON_PATUX","NASA_LaRC", "EPA-Res_Triangle_Pk", "Georgia_Tech"),]
sd(wdb.f.e$PM25)
```
...
#clean working database to complete cases
setDF(wdb.f.w)
dataset <- wdb.f.w[,  names(wdb.f.w)%in%vars$Var1]
...
## Plot var importance based on SHAP
var_importance(shap_result$mean_shap_score, top_n=10)

## Prepare data for top N variables
shap_long <- shap.prep(shap = shap_result$mean_shap_score,
                           X_train = data_x , 
                           top_n = 10)
...
tiff(paste0(res_dir,"shap_PMAODINV_CA.tif"), res = 300)
plot.shap.summary(shap_long)#Plot shap overall metrics
dev.off()
