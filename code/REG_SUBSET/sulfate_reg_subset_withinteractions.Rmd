---
title: "sulfate - CA Central Valley - Find best subset of variables for multivariate regression"
...
patterns <- c("Asymmetry_Factor","AOD","sulfate","Angstrom_Exponent","Precipitable_Water(cm)","Solar_Zenith_Angle(Degrees)","REff")
vars.new <- filter(vars, grepl(paste(patterns, collapse="|"), Var1))
vars.new <- vars.new[,"Var1"]
vars.new <- c(vars.new,"AOD_550")
vars.new <- c(vars.new,"sulfate")
```
...
aod.dataset <- na.omit(inv.dataset[,c("sulfate","AOD_550","Site_Name")])
...
print(paste0("R2 = ",cor(aod.dataset$sulfate,aod.dataset$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.dataset$sulfate~aod.dataset$AOD_550))))

#Fresno
print(paste0("Fresno N = ",dim(aod.fresno)[1]))
print(paste0("R2 = ",cor(aod.fresno$sulfate,aod.fresno$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.fresno$sulfate~aod.fresno$AOD_550))))

#Bakersfield
print(paste0("Bakersfield N = ",dim(aod.baker)[1]))
print(paste0("R2 = ",cor(aod.baker$sulfate,aod.baker$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.baker$sulfate~aod.baker$AOD_550))))
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.dataset$sulfate[folds == j] - pred)^2))
    r2[j,i] = cor(inv.dataset$sulfate[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.sulfate.csv"))
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.fresno$sulfate[folds == j] - pred)^2))
    r2[j,i] = cor(inv.fresno$sulfate[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.sulfate.fresno.csv"))
```
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.baker$sulfate[folds == j] - pred)^2))
    r2[j,i] = cor(inv.baker$sulfate[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.sulfate.baker.csv"))
```
