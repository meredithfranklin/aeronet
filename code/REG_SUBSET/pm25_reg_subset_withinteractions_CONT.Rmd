...
#AQS-Contonuous
wdb.inv<-fread("C:/Users/msorekha/Documents/Projects/JPL/MISR/Data/WDB/AOD.pm.inv.CONT.csv")
...
inv.dataset <- inv.dataset[,colSums(is.na(inv.dataset))/dim(inv.dataset)[1]<0.4]
setDF(inv.dataset)
...
inv.dataset <- na.omit(inv.dataset)#1978

inv.fresno <- inv.dataset[Site_Name=="Fresno",]#1528
inv.fresno <- inv.fresno [,-c("Site_Name")]
inv.baker<- inv.dataset[Site_Name=="Bakersfield",]#95
inv.baker <- inv.baker [,-c("Site_Name")]

aod.dataset <- na.omit(inv.dataset[,c("PM25","AOD_550","Site_Name")])
aod.fresno <- aod.dataset[Site_Name=="Fresno",]
aod.fresno <- aod.fresno [,-c("Site_Name")]
aod.baker <- aod.dataset[Site_Name=="Bakersfield",]
aod.baker <- aod.baker [,-c("Site_Name")]

inv.dataset <- inv.dataset[,-c("Site_Name")]
aod.dataset <- aod.dataset[,-c("Site_Name")]
```
#stats PM25
```{r}
