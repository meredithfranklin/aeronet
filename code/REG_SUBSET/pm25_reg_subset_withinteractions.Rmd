...
wdb.inv <-fread("/Volumes/GoogleDrive/My Drive/AERONET-MISR/Paper#2-RSoE/aeronet/aeronet/data/inv.pm.data.csv")

# west
wdb.inv.w <-wdb.inv[wdb.inv$Site_Name %in% c("Fresno_2", "Fresno","DRAGON_Madera_city", "DRAGON_Tranquility","DRAGON_Bakersfield", "DRAGON_Garland", "Modesto"),]
dim(wdb.inv.w)

# central
wdb.inv.c <-wdb.inv[wdb.inv$Site_Name %in% c("Denver_LaCasa","DRAGON_Denver_LaCasa", "DRAGON_Chatfield_Pk", "NEON_RNMP"),]
dim(wdb.inv.c)

# St Louis (optional)
wdb.inv.c2 <-wdb.inv[wdb.inv$Site_Name %in% c("St_Louis_University"),]
dim(wdb.inv.c2)

# east
wdb.inv.e <-wdb.inv[wdb.inv$Site_Name %in% c("USDA_Howard","DRAGON_LAREL","DRAGON_PATUX","NASA_LaRC", "DRAGON_Essex","Big_Meadows","CCNY"),]
dim(wdb.inv.e)
```
