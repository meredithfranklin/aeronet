---
title: "OC - CA Central Valley - Find best subset of variables for multivariate regression"
...
wdb.aod<-fread("/Volumes/GoogleDrive/My Drive/AERONET-MISR/Paper#2-RSoE/data/AODonly.pm.CSN.csv")
wdb.aod$V1<-NULL

wdb.inv<-fread("/Volumes/GoogleDrive/My Drive/AERONET-MISR/Paper#2-RSoE/data/AOD.pm.inv.CSN.csv")
...
patterns <- c("Asymmetry_Factor","AOD","OC","Angstrom_Exponent","Precipitable_Water(cm)","Solar_Zenith_Angle(Degrees)","REff")
vars.new <- filter(vars, grepl(paste(patterns, collapse="|"), Var1))
vars.new <- vars.new[,"Var1"]
vars.new <- c(vars.new,"AOD_550")
vars.new <- c(vars.new,"OC")
```
...
inv.dataset <- inv.dataset[,colSums(is.na(inv.dataset))/dim(inv.dataset)[1]<0.76]
setDF(inv.dataset)
...
aod.dataset <- na.omit(inv.dataset[,c("OC","AOD_550","Site_Name")])
...
print(paste0("R2 = ",cor(aod.dataset$OC,aod.dataset$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.dataset$OC~aod.dataset$AOD_550))))

#Fresno
print(paste0("Fresno N = ",dim(aod.fresno)[1]))
print(paste0("R2 = ",cor(aod.fresno$OC,aod.fresno$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.fresno$OC~aod.fresno$AOD_550))))

#Bakersfield
print(paste0("Bakersfield N = ",dim(aod.baker)[1]))
print(paste0("R2 = ",cor(aod.baker$OC,aod.baker$AOD_550)^2))
print(paste0("RMSE = ",rmse(lm(aod.baker$OC~aod.baker$AOD_550))))
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.dataset$OC[folds == j] - pred)^2))
    r2[j,i] = cor(inv.dataset$OC[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.OC.csv"))
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.fresno$OC[folds == j] - pred)^2))
    r2[j,i] = cor(inv.fresno$OC[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.OC.fresno.csv"))
```
...
for(j in 1:k){
  ...
  for (i in 1:(max.dim)){
    ...
    cv.errors[j, i] = sqrt(mean((inv.baker$OC[folds == j] - pred)^2))
    r2[j,i] = cor(inv.baker$OC[folds == j] , pred)^2
  }
}
...
write.csv(var.mat,paste0(res_dir,"var.mat.OC.baker.csv"))
```
